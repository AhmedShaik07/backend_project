...
const updateUserAvatar = asyncHandler( async (req, res) => {
    ...
    if(!avatarLocalPath){
        throw new ApiError(400,"No avatar file is uploaded.!")
    }
...
const updateCoverImage = asyncHandler( async (req, res) => {
    ...
    if(!coverImageLocalPath){
        throw new ApiError(400,"No cover image file is uploaded.!")
    }

    const coverImage = await uploadOnCloudinary(coverImageLocalPath)

    if(!coverImage.url){
        throw new ApiError(400, "Error while uploading new cover image on cloudinary")
    }
